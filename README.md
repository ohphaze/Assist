...
## Build (Local)
- Prerequisites: Install .NET SDK `8.0.x` (SDK, not just runtime). Visual Studio 2022 or VS Code + C# tools recommended.
- Submodules: This repo depends on `Assist.Shared` as a Git submodule. If you cloned without submodules, run `git submodule update --init --recursive` or clone with `--recurse-submodules`. If you downloaded a ZIP, fetch `https://github.com/AssistTeam/Assist.Shared` and place it at `Assist.Shared/Assist.Shared/` so the solution reference resolves.
- Restore/build: From `Assist/`, run `dotnet restore` then `dotnet build -c Release`. The packages are pinned to major versions but will float to the latest compatible patch to keep you up to date.
- WebView2: Ensure the Microsoft Edge WebView2 Runtime is installed (required by the account auth view). On most Windows machines with Edge installed, this is already present.
